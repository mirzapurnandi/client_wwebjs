    ...
    <div class="card">
        ...
        <div class="card-body table-responsive p-0 " :class="{ 'overlay-wrapper': this.processing }">
            ...
            <table class="table table-hover text-nowrap">
                ...
                <tbody>
                    <tr v-for="(provider, index) in providers" :key="index" :class="`provider_${provider.id}`">
                        <td>{{ index + 1 }}</td>
                        <td>{{ provider.name }}</td>
                        ...
                        <td>{{ provider.status == true ? 'Active' : 'Non-Active' }}</td>
                        <td>edit | hapus </td>
                    </tr>
                </tbody>
            </table>
...
export default {
    ...
    computed: {
        ...mapState(['processing']),
        ...mapGetters('provider', ['providers', 'pagination'])
    },
